...
int main(int, char**)
{
	...
	g::ideLocation = fs::current_path().string();
    ...
    if (settings.settingData.contains("last_project") && settings.settingData["last_project"].is_string())
        g::currentProjectPath = settings.settingData["last_project"].get<std::string>();

    if (settings.settingData.contains("last_file") && settings.settingData["last_file"].is_string())
        g::activeFilePath = settings.settingData["last_file"].get<std::string>();

    if (!g::activeFilePath.empty() && fs::exists(g::activeFilePath))
    {
        std::ifstream t(g::activeFilePath);
        if (t.good())
        {
            std::string str((std::istreambuf_iterator<char>(t)), std::istreambuf_iterator<char>());
            g::editor.SetText(str);
        }
    }
    ...
    settings.settingData["last_project"] = g::currentProjectPath;
    settings.settingData["last_file"] = g::activeFilePath;
    settings.save();
